...
The function accepts the same parameters as `@openapi(...)` (path, method, operation_id, summary, request/response schemas, etc.) and writes directly to the shared `_openapi_registry`. Once registered, routes appear in the generated spec alongside decorator-registered routes.
...
- Runtime execution or graph deployment (owned by `azure-functions-langgraph`)
